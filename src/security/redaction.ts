 ...
 * When multiple patterns could match the same string, the first match wins
 * because each pattern applies a unique replacement token that prevents
 * subsequent patterns from re-matching.
 *
 * Patterns with /g flag are recreated per-call to avoid lastIndex state bugs.
 */
export const REDACTION_PATTERNS: RedactionPattern[] = [
  // ── Private keys (most specific, multi-line) ──
  {
    name: 'private_key',
  ...
  // NOTE: The old aws_secret_key pattern /\b([0-9a-zA-Z/+]{40})\b/ was far too broad
  // and matched ANY 40-char base64 string (UUIDs, hashes, legitimate data).
  // AWS secret keys are 40-char base64 strings typically appearing near an AKIA prefix.
  // We now only flag them when adjacent to an AWS access key or explicit aws_secret context.
  {
    name: 'aws_secret_key',
    pattern: /(?:AKIA[0-9A-Z]{16}[^]*?)\b([0-9a-zA-Z/+]{40})\b/g,
  ...
  // Gemini MUST come before GCP API key — both match AIzaSy prefix,
  // but Gemini is the more specific pattern.
  {
  ...
  {
    name: 'private_ip',
    pattern: /\b((?:10\.(?:\d{1,3})\.){2}\d{1,3}|(?:172\.(?:1[6-9]|2\d|3[01])\.){2}\d{1,3}|(?:192\.168\.\d{1,3}\.)\d{1,3})\b/g,
...
export function redactString(value: string): { result: string; flags: RedactionFlag[] } {
  ...
  for (const pattern of REDACTION_PATTERNS) {
    // Recreate regex to avoid /g flag lastIndex issues on shared pattern objects
